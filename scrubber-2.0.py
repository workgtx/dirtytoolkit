...
class Users():
    ...
    def votes_cycle_f(self):
        try:
            ...
            print ('Ошибка вызова функции \'getdirty.users_f()\', проверьте аргументы')
        self.maxvalue = 10000
        self.iter = 0
        self.a = False
        self.b = False
    ...
    def userdata_to_db_f(self):
        if self.a == False:
            try:
                self.userdataup = pushintodb.users(self.userupvoters[self.iter][1],self.userupvoters[self.iter][2],self.userupvoters[self.iter][4],self.userupvoters[self.iter][5],self.userupvoters[self.iter][6],boolify_f(self.userupvoters[self.iter][7]),boolify_f(self.userupvoters[self.iter][8]),boolify_f(self.userupvoters[self.iter][9]))
            except: 
                self.a = True
            try:
        ...
        if self.b == False:
            try:
                self.userdatadown = pushintodb.users(self.userdownvoters[self.iter][1],self.userdownvoters[self.iter][2],self.userdownvoters[self.iter][4],self.userdownvoters[self.iter][5],self.userdownvoters[self.iter][6],boolify_f(self.userdownvoters[self.iter][7]),boolify_f(self.userdownvoters[self.iter][8]),boolify_f(self.userdownvoters[self.iter][9]))
            except: 
                self.b = True
            try:
    ...
    def uservotes_to_db_f(self):
        try:
            self.uservotes = pushintodb.UsersVotes(self.index, self.userupvoters, user.downvoters)
        except: 
