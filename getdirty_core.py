...
class GrabData:
    ...
    def url_gen_f (self):
        __b={   1:{1:'https://d3.ru/api',2:'domains',3:'posts',4:'votes',5:'comments',6:'users'},\
                2:{1:'per_page',2:'page',3:'sorting'},\
                3:{1:'hotness',2:'rating',3:'date_created',4:'date_changed'},\
                4:{1:'/',2:'?',3:'&',4:'='}}
        __url={ 'domains':'%s%s%s%s%s%s%d%s%s%s%d' \
                ...
                __b[2][1], '=', self.per_page, '&', __b[2][3], '=', __b[3][3]),
                'p_votes': '%s%s%s%s%s%s%s%s%s%s%d%s%s%s%d' \
                % (__b[1][1], '/', __b[1][3], '/', self.index, '/', __b[1][4], '?', \
                __b[2][2], '=', self.page, '&', __b[2][1], '=', self.per_page),
                'comments':'%s%s%s%s%s%s%s%s%s%s%d%s%s%s%d' \
                % (__b[1][1], '/', __b[1][3], '/', self.index, '/', __b[1][5], '?', \
                __b[2][2], '=', self.page, '&', __b[2][1], '=', self.per_page),
                'c_votes': '%s%s%s%s%s%s%s%s' \
                % (__b[1][1], '/', __b[1][5], '/', self.index, '/', __b[1][4], '/'),\
                'users': '%s%s%s%s%s%s%s%s%s%s%s' \
                % (__b[1][1], '/', __b[1][6], '/', self.index, '/', __b[1][3], '?',\
                __b[2][1], '=', self.per_page), \
                'u_votes': '%s%s%s%s%s%s%s%s' \
                % (__b[1][1], '/', __b[1][6], '/', self.index, '/', __b[1][4], '/')} \
                [self.act]
        ...
        if i == 1: 
            try:
                ...
                if self.page == 1:
                    ...
                    self.upvoters = '%ssephereseplineheresephere%s' % (self.upvoters, self.parsed)
            except Exception:
                return('')
            ...
            try:
                ...
                if self.page == 1:
                    ...
                    self.downvoters = '%ssephereseplineheresephere%s' % (self.downvoters, self.parsed)
            except Exception:
                return('')
    ...
    def prepare_f (self):
        __struct={'pgc':'''"page_count","item_count"''',
                  ...
                  "url"]], "comments_count","tags", "golden", "can_delete",\
                  "can_unpublish","can_edit","can_moderate","created"]''',\
                  'upvoters':'''+"upvotes"["vote",."user"["id","login"],"changed",\
                  ."user"["gender","rank","karma","is_golden","active","deleted"]]''',\
                  ...
                  "is_readable_for_everyone",."owner"["login"],."president"["login"]]''',\
                  'comments':'''+"comments"["id","parent_id","tree_level","rating",."user"["id","login"],\
                  "body","can_delete","can_remove_comment_threads","can_edit","can_moderate","created"]'''}
        __pg_mask = JsonToCsv(__struct[self.method], nullValue = 'null', debug = False) 
        __pg_extr = __pg_mask.extractData(self.response)
        __pg_pars = __pg_mask.dataToStr(__pg_extr,\
        separator = 'sephere', lineSeparator = 'sephereseplineheresephere')
